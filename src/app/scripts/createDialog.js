  ...
  function ($document, $compile, $rootScope, $controller, $timeout) {
    ...
    return function Dialog(templateUrl/*optional*/, options, passedInLocals) {
      ...
      var modalEl = angular.element(
                '<div ng-cloak class="' + options.modalClass + ' fade"' + idAttr + ' style="display: block;">' +
                    '  <div class="modal-dialog">' +
                    '    <div class="modal-content">' +
                    '      <div class="modal-header clearfix">' +
                    '        <a class="close" ng-click="$modalCancel()"></a>' +
                    '        <h2>{{$title}}</h2>' +
                    '      </div>' +
                    modalBody +
                    footerTemplate +
      ...
      scope.$modalSuccess = function () {
        ...
        var flag = callFn.call(this) == undefined || callFn.call(this);
        if(flag){
	        scope.$modalClose();
	
	        // angular.element('body').css({'height':'auto','overflow':'auto'});
	        hideScroll(false);
        };
      };
      ...
      if (options.controller) {
        ...
        modalEl.contents().data('$ngControllerController', ctrl);
          ...
          if(options.preload) options.preload(scope);
      }
